...
def load_product_data():
    ...
    try:
        df = pd.read_csv('amazon_products.csv')  # Update this path to your CSV file location
        return df
...
def search_products(query, df, limit=5):
    """
    ...
    for _, row in df.iterrows():
        ...
        # Check product name
        if isinstance(row.get('product_name'), str) and query_lower in row['product_name'].lower():
            relevance += 10
        
        # Check category
        if isinstance(row.get('category'), str) and query_lower in row['category'].lower():
            relevance += 5
        
        # Check product description
        if isinstance(row.get('about_product'), str) and query_lower in row['about_product'].lower():
            relevance += 3
            
        # Check review content
        if isinstance(row.get('review_content'), str) and query_lower in row['review_content'].lower():
            relevance += 2
        ...
        if relevance > 0:
            results.append({
                ...
                'discount_percentage': row.get('discount_percentage', 'N/A'),
                'rating': row.get('rating', 'N/A'),
                'rating_count': row.get('rating_count', 'N/A'),
                'about_product': row.get('about_product', 'N/A'),
                'review_title': row.get('review_title', 'N/A'),
                'review_content': row.get('review_content', 'N/A'),
                'relevance': relevance
            })
...
def get_product_context(query, df):
    """
    ...
    relevant_products = search_products(query, df)
    ...
    for i, product in enumerate(relevant_products, 1):
        ...
        context += f"Pricing: ₹{product['discounted_price']} (Original: ₹{product['actual_price']}, Discount: {product['discount_percentage']}%)\n"
        context += f"Rating: {product['rating']}/5 based on {product['rating_count']} reviews\n"
        context += f"Description: {product['about_product']}\n"
        context += f"Review: {product['review_title']} - {product['review_content']}\n\n"
...
def process_message(user_message):
    ...
    if is_product_query(user_message) and not st.session_state.product_data.empty:
...
if not st.session_state.product_data.empty:
    ...
    st.sidebar.write(f"Products: {st.session_state.product_data['product_id'].nunique() if 'product_id' in st.session_state.product_data.columns else 'N/A'}")
    st.sidebar.write(f"Categories: {st.session_state.product_data['category'].nunique() if 'category' in st.session_state.product_data.columns else 'N/A'}")